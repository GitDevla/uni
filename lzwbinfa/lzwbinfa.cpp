...
// tagjait nem használja direktben 0.0.6.2      Kis kommentezést teszünk bele 1.
// lépésként (hogy a kicsit lemaradt hallgatóknak is
...
// http://progpater.blog.hu/2011/04/17/a_tizedik_tizenegyedik_labor 0.0.6.4
// SVN-beli, http://www.inf.unideb.hu/~nbatfai/p1/forrasok-SVN/bevezetes/vedes/
// 0.0.6.5      2012.03.20, z3a4.cpp: N betűk (hiányok), sorvégek, vezető
// komment figyelmen kívül:
// http://progpater.blog.hu/2012/03/20/a_vedes_elokeszitese 0.0.6.6 z3a5.cpp:
// mamenyaka kolléga észrevételére a több komment sor figyelmen kívül hagyása
...
// további komment a << 		eltoló operátort tagfüggvényként,
// illetve globális függvényként túlterhelő részekhez.
...
class LZWBinFa {
   public:
    /* Szemben a bináris keresőfánkkal (BinFa osztály)
     ...
     tartalmazó objektum, lásd majd a védett tagok között lent: Csomopont
     gyoker; A fa viszont már pointer, mindig az épülő LZW-fánk azon
     csomópontjára mutat, amit az input feldolgozása során az LZW algoritmus
     logikája diktál: http://progpater.blog.hu/2011/02/19/gyonyor_a_tomor Ez a
     konstruktor annyit csinál, hogy a fa mutatót ráállítja a gyökérre. (Mert
     ugye laboron, blogon, előadásban tisztáztuk, hogy a tartalmazott tagok,
     most "Csomopont gyoker" konstruktora előbb lefut, mint a tagot tartalmazó
     LZWBinFa osztály konstruktora, éppen a következő, azaz a fa=&gyoker OK.)
   */
    LZWBinFa() : fa(&gyoker) {}
    ~LZWBinFa() {
    ...
    /* Tagfüggvényként túlterheljük a << operátort, ezzel a célunk, hogy
     felkeltsük a hallgató érdeklődését, mert ekkor így nyomhatjuk a fába az
     inputot: binFa << b; ahol a b egy '0' vagy '1'-es betű. Mivel tagfüggvény,
     így van rá "értelmezve" az aktuális (this "rejtett paraméterként" kapott )
     példány, azaz annak a fának amibe éppen be akarjuk nyomni a b betűt a
     tagjai (pl.: "fa", "gyoker") használhatóak a függvényben.
     ...
     a binFa << b (ahol a b majd a végén látszik, hogy már az '1' vagy a '0')
     azt jelenti tagfüggvényként, hogy binFa.operator<<(b) (globálisként így
     festene: operator<<(binFa, b) )
    ...
    /* A bejárással kapcsolatos függvényeink (túlterhelt kiir-ók, atlag, ratlag
     stb.) rekurzívak, tk. a rekurzív fabejárást valósítják meg (lásd a 3.
     előadás "Fabejárás" c. fóliáját és társait)

     (Ha a rekurzív függvénnyel általában gondod van => K&R könyv megfelelő
     része: a 3. ea. izometrikus részében ezt "letáncoltuk" :) és külön idéztük
     a K&R álláspontját :)
   */
    void kiir(void) {
        // Sokkal elegánsabb lenne (és más, a bevezetésben nem kibontandó
        // reentráns kérdések miatt is, mert ugye ha most két helyről hívják meg
        // az objektum ilyen függvényeit, tahát ha kétszer kezd futni az
        // objektum kiir() fgv.-e pl., az komoly hiba, mert elromlana a
        // mélység... tehát a mostani megoldásunk nem reentráns) ha nem
        // használnánk a C verzióban globális változókat, a C++ változatban
        // példánytagot a mélység kezelésére:
        // http://progpater.blog.hu/2011/03/05/there_is_no_spoon
        melyseg = 0;
        // ha nem mondta meg a hívó az üzenetben, hogy hova írjuk ki a fát,
        // akkor a sztenderd out-ra nyomjuk
  ...
  */
    ...
    /* Vágyunk, hogy a felépített LZW fát ki tudjuk nyomni ilyenformán:
     std::cout << binFa; de mivel a << operátor is a sztenderd névtérben van, de
     a using namespace std-t elvből nem használjuk bevezető kurzusban, így ez a
   ...
   private:
    class Csomopont {
       public:
        /* A paraméter nélküli konstruktor az elepértelmezett '/'
       "gyökér-betűvel" hozza létre a csomópontot, ilyet hívunk a fából, aki
       tagként tartalmazza a gyökeret. Máskülönben, ha valami betűvel hívjuk,
       akkor azt teszi a "betu" tagba, a két gyermekre mutató mutatót pedig
       nullra állítjuk, C++-ban a 0 is megteszi. */
        Csomopont(char b = '/')
            : betu(b), balNulla(nullptr), jobbEgy(nullptr){};
        ~Csomopont(){};
       ...
       private:
        ...
        // metódusai nem közvetlenül a Csomopont tagjaival dolgoznak, hanem
        // beállító/lekérdező üzenetekkel érik el azokat */

        // Milyen betűt hordoz a csomópont
        char betu;
        // Melyik másik csomópont a bal oldali gyermeke? (a C változatból
        // "örökölt" logika: ha hincs ilyen csermek, akkor balNulla == null)
        // igaz
        Csomopont *balNulla;
        Csomopont *jobbEgy;
        // nem másolható a csomópont (ökörszabály: ha van valamilye a szabad
        // tárban, letiltjuk a másoló konstruktort, meg a másoló értékadást)
   ...
   protected:  // ha esetleg egyszer majd kiterjesztjük az osztályt, mert
    ...
    // máshogy... stb. akkor ezek látszanak majd a gyerek osztályban is
...
// sablonosztályosított tovább fejlesztésben az linkelési gondot okozna, de ez a
// téma már kivezet a laborteljesítés szükséges feladatából:
// http://progpater.blog.hu/2011/04/12/imadni_fogjak_a_c_t_egy_emberkent_tiszta_szivbol_3
...
fájlokkal kell dolgozni int main ()
{
    char b;
    LZWBinFa binFa;
...
int main(int argc, char *argv[]) {
    ...
    // a kiírás szerint ./lzwtree in_file -o out_file alakra kell mennie, ez 4
    // db arg:
    if (argc != 4) {
    ...
    // hogy meg tudjuk majd nézni... :) l. az említett 5. ea. C -> C++
    // gyökkettes átírási példáit
    ...
    while (beFile.read((char *)&b, sizeof(unsigned char))) {
        ...
        // for (int i = 0; i < 8; ++i) {  // foggalmam sincs, hogy miért kell
